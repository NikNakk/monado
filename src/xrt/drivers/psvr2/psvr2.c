...
struct psvr2_pending_horizon_prediction
{
	...
	struct xrt_vec3 acceleration_predicted_position;
...
psvr2_usb_thread(void *ptr)
{
	 ...
	 * one event-handling pass is not sufficient on macOS and can leave Darwin
	 * callbacks referring to freed transfers during libusb_exit().
	 */
	int attempts = 0;
	os_thread_helper_lock(&hmd->usb_thread);
