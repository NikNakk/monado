                                         ...
                                         uint64_t presentation_monotonic_ns)
{
	...
	if (hybrid) {
		 ...
		 * the compositor is behind, wait_tick() consumes the newest published tick
		 * and intentionally skips older callbacks. The callback timestamps are not
		 * used to replace the native compositor's HMD prediction/presentation time. */
		pthread_mutex_unlock(&g_bridge.mutex);
		return true;
	}
                                       ...
                                       uint64_t *out_presentation_monotonic_ns)
{
	...
	for (;;) {
		...
		/* Hybrid may publish several ticks while the compositor is busy. Consume
		 * the newest one: CAMetalDisplayLink is an event source, not a queued clock. */
...
{
	/* Hybrid deliberately uses CAMetalDisplayLink only to release the compositor
	 * wait. Native HMD pacing remains authoritative for prediction and presentation. */
